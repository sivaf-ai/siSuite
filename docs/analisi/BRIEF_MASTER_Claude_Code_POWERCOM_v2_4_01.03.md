...
> Lingua: **italiano** per UI/testi/commenti; **inglese** per gli identificatori di codice. Chat 01.03 · 16/06/2026 · **v2.4 — migrazione liste & CRUD esistenti allo standard v2 (Blocco M)**.
...
- **Blocco B — Ordini di lavoro (costruito come "Ordinativi FTTH"): FATTO** (maschera "metro", mock 44): lista + scheda, **PII mascherata server-side**, RBAC (`work_order:*`, `serial:*`, `pii:read`), import CSV con dedup, voce menu (a **1 livello**). Riusati i componenti del design system esistente.
...
- **Non rinominare la tabella fisica** `company` (referenziata ovunque: `engagement`, `asset`, `work_order.principal_company_id`, `price_list_override`, `stock_serial_unit`, `company_role/contact`, `app_user`, RLS `app_current_company`). Si cambia solo concetto + etichette + i18n.
- **i18n vincolante** (tre lingue, da seedare nei namespace i18next / glossario):
  ...
  | Ordine di lavoro (entità `work_order`) | Ordine di lavoro | Work order | Orden de trabajo |
  | Tipo ordine (`work_order_type`) | Tipo ordine | Work order type | Tipo de orden |
  | Committente esterno (`principal_company_id`) | Committente | Principal | Comitente |
...
11. **"Ordinativo (FTTH)" → "Ordine di lavoro" (generico).** Entità `work_order` invariata nel codice (standard di mercato come Salesforce/Dynamics). **Etichetta base IT "Ordini di lavoro"**, sovrascrivibile per-tenant via glossario (FTTH→"Attivazioni", manutenzione→"Interventi"). **Rename fisico** (migrazione `032`): `operator_company_id`→**`principal_company_id`** (committente esterno), `operator_order_id`→**`principal_order_ref`**. **Nuova colonna `type_id`** → `lookup_value('work_order_type')` (FTTH seed: "Attivazione"). Tolto il badge "FTTH" dal menù; corretto il commento tabella. Code aggiorna il codice del Blocco B che referenzia le colonne rinominate. (ADR-0006)
12. **Configuratore campi (Field Builder) in Impostazioni:** UI + endpoint per gestire `field_definition` **per tenant ed entità/verticale**, così i campi specifici (oggi i 5 FTTH della 029) si configurano senza toccare il codice. È il meccanismo che rende il software parametrizzabile da subito. Spec in Parte 8 (Blocco A-bis). (ADR-0006)
13. **Migrazione liste & CRUD esistenti allo standard v2 (CRITICO, non opzionale):** TUTTE le liste e i CRUD già in app (oggi ancora su `DataTable`/`Drawer` + icone-azione sulle righe, pattern del vecchio `FRONTEND_SPEC.md`) vanno **rifatti** sui componenti `EntityList`/`ObjectPage` estratti, secondo lo **standard v2** (mock 41/43). Dove il vecchio `FRONTEND_SPEC.md` confligge col v2, **vince il v2** (drawer master-detail e icone-azione sulle righe = superati). Spec e inventario in Parte 8 (Blocco M).
...
2. **Blocco A-bis** — **Configuratore campi (Field Builder)** in Impostazioni (CRUD `field_definition` per tenant/entità).
3. **Blocco B-ter** — **Generalizzazione "Ordine di lavoro"**: migrazione `032` (rename colonne + `type_id`), relabel UI/i18n, via il badge FTTH.
4. **Blocco M** — **Migrazione liste & CRUD esistenti allo standard v2** (Soggetti, Commesse, Asset, Risorse, Utenti, Stati, Numerazioni, Catture…). *Critico, dipende dai componenti del Blocco A.*
5. **Blocco C** — Articoli & seriali.
6. **Blocco C-bis** — Anagrafica **Siti/Località** (entità `site` gerarchica) + `asset.site_id`.
7. **Seed pack fibra (minimo)** — per la demo.
8. **Blocco B-bis** — rifinitura Ordini di lavoro.
9. **Blocco D** — Listino + `resolvePrice`.
10. **Blocco E** — Lavorazioni + libretto.
11. **Blocco F** — Rapportino + CaptureBarAI.
12. **Blocco G** — Pivot + export.
13. **Blocco H** — Magazzino/DDT + seed completo.

> Sequenza minima per la **demo**: A (menù+componenti) → M-priorità ALTA (Soggetti, Commesse) → seed → Ordini di lavoro (fatto) → C (seriali/parco installato) → G (pivot). Le voci **BASSA** del Blocco M (Stati, Numerazioni, Piano) possono slittare dopo la demo.
...
- **Sezioni:** `Preferiti` · **LAVORO** (Cruscotto, Commesse, Campo, Magazzino, Finanza & Budget) · **DATI** (Anagrafiche: Soggetti, Articoli, Risorse, Materiali) · **SISTEMA** (Impostazioni, Amministrazione, SuperUsers). Ordini di lavoro sotto **Campo**.
...
## ⬛ Blocco A-bis — Configuratore Campi (Field Builder) in Impostazioni

**Obiettivo:** dare all'admin un posto per **creare/modificare i campi personalizzati** (`field_definition`) per il suo tenant, per entità e verticale, senza toccare il codice. È il motore della strategia metadata-driven. I 5 campi FTTH della 029 sono l'esempio "di sistema" già caricato.

**Backend (`field_definition` CRUD — finora mancava):**
- `GET /field-definitions?entity=…&vertical=…` — righe di sistema (`tenant_id NULL`, sola lettura) + override del tenant.
- `POST/PATCH/DELETE /field-definitions` — **solo righe del tenant corrente** (le righe di sistema non si toccano: si "sovrascrivono" creando una riga tenant con stesso `entity`+`key`). Validazione: `key` univoco per `(tenant,entity,key)`; `data_type` in set ammesso; `options` obbligatorio se `data_type` ∈ {select,multiselect}.
- RBAC: nuovo permesso **`field_definition:manage`** (Owner/Admin).

**Frontend — Impostazioni › "Campi personalizzati":**
- Selettore **Entità** (Ordine di lavoro · Articolo · Soggetto · Asset · Sito · Commessa…) + opz. **Verticale/Tipo**.
- **Lista campi** raggruppati per `group_key`, **ordinabili in drag** (= `sequence`); badge "di sistema" (read-only) vs "personalizzato"; toggle `active`.
- **Drawer editor campo**: `key` (slug), **label i18n (it/en/es-AR)**, `help`, `data_type` (text/number/integer/select/multiselect/date/datetime/bool/currency…), `required`, `options` (per select), `validation` (min/max/regex), `unit`, `group_key`, `placeholder` i18n, `sequence`, `active`.
- **Anteprima live**: poiché `EntityForm` rende dai `field_definition`, mostra a fianco il form risultante (così l'admin "vede" subito l'effetto).

**DoD:** l'admin apre Impostazioni › Campi personalizzati, seleziona "Ordine di lavoro", **vede i 5 campi fibra** (di sistema), ne aggiunge uno nuovo del tenant (es. "Note POP", text), lo riordina, e il nuovo campo **compare** nella scheda Ordine di lavoro (rendering da `EntityForm`). Le righe di sistema non sono cancellabili. RLS per tenant.

## ⬛ Blocco B-ter — Generalizzazione "Ordine di lavoro"

**Migrazione `032_work_order_generalize.sql` (alter, additiva):**
```sql
-- 1) rename colonne troppo specifiche → committente esterno generico
ALTER TABLE work_order RENAME COLUMN operator_company_id TO principal_company_id;
ALTER TABLE work_order RENAME COLUMN operator_order_id   TO principal_order_ref;
-- aggiorna i nomi di FK e UNIQUE per pulizia (le colonne nel vincolo seguono il rename):
ALTER TABLE work_order RENAME CONSTRAINT work_order_operator_company_id_fkey TO work_order_principal_company_id_fkey;
ALTER TABLE work_order RENAME CONSTRAINT work_order_tenant_id_operator_company_id_operator_order_id_key TO work_order_principal_ref_uk;
-- 2) tipo configurabile
ALTER TABLE work_order ADD COLUMN type_id uuid REFERENCES lookup_value(id);
-- 3) seed canonical_state + lookup_value categoria 'work_order_type' (sistema, tenant_id NULL),
--    code 'activation' label {it:'Attivazione', en:'Activation', es-AR:'Activación'} (pattern migrazione 028)
-- 4) aggiorna il COMMENT della tabella in versione generica (ordine di lavoro, non "FTTH")
```
**Codice (Code aggiorna il Blocco B già fatto):** `entities.ts`, `routes/workOrders.ts`, `OrdinativiPage`/`OrdinativoDetailPage` → rinominare i riferimenti a `operator_*` in `principal_*`; aggiungere `type_id` (select da lookup). **Import CSV**: la dedup ora è su `(tenant, principal_company_id, principal_order_ref)`.
**UI/i18n:** etichetta base **"Ordini di lavoro"** (Parte 4); via il badge "(FTTH)" dal menù; colonne mostrate come **"Committente"** / **"Rif. esterno"**; selettore **"Tipo"**.
**DoD:** lo schema mostra `principal_company_id/principal_order_ref/type_id`; il Blocco B funziona identico con i nuovi nomi; il menù dice "Ordini di lavoro" senza FTTH; in scheda compare il campo "Tipo".

## ⬛ Blocco M — Migrazione liste & CRUD esistenti allo standard v2 (CRITICO)

**Perché.** Lo standard Lista/Scheda v2 (mock `41`/`43`) è il target, ma le liste/CRUD **già in app** sono ancora sul pattern vecchio (`DataTable` + `Drawer` + **icone-azione sulle righe**, dal vecchio `FRONTEND_SPEC.md`). Vanno **tutte rifatte** sui componenti `EntityList`/`ObjectPage` **estratti nel Blocco A**. **Dove il vecchio `FRONTEND_SPEC.md` confligge col v2, vince il v2** (drawer master-detail e icone-azione sulle righe sono **superati**). Dipende dal Blocco A (componenti estratti).

**Cosa cambiare, in concreto, su OGNI lista esistente** (checklist v2):
1. Passare da `DataTable`/`Drawer` → **`EntityList`** + **`ObjectPage`** estratti.
2. **Togliere le icone-azione dalle righe.** Le azioni vivono nella **toolbar a sole icone + tooltip**; la **selezione mostra solo il conteggio**; il **click sulla riga apre la scheda** (ObjectPage).
3. **Viste** (filtri salvati) sulla **riga del titolo**; **ricerca** che si espande; **Filtri / Colonne / Azioni AI**; gruppo azioni dati a destra con **Nuovo "+" per ultimo**.
4. Righe a **2 livelli** per entità ricche (dato primario sopra, collegato sotto in corsivo); **1 livello** per griglie/poche-colonne.
5. Numeri **a destra/tabellari**, valuta **contabile**, durata **h:mm**, orario 24h.

**Cosa cambiare su OGNI scheda/CRUD esistente** (checklist v2):
6. Crea + vedi + modifica in **un'unica `ObjectPage`** (non più drawer): **header sticky opaco** con **solo Salva/Annulla** (Duplica/Elimina sono azioni di **lista**, non vanno nella scheda; niente dati del record ripetuti nell'header).
7. **Label e titoli box nel bordo** (statici); griglia densa, campi affiancati.
8. **Validazione dentro il campo** (bordo rosso + messaggio), non in cima.
9. Tabelle correlate come **tab strip in fondo**; azioni AI contestuali nel **bordo del box**; tooltip su tutte le icone.
10. Campi **da `field_definition`** (EntityForm); ID da `number_series`; stati da `lookup_value`; **solo design token**; icone **lucide**; **RBAC su UI *e* API**; stati **vuoto/caricamento/errore**; **responsive**.

**Inventario da migrare (entità reali, da `FRONTEND_SPEC §4`):**
| Entità | Etichetta UI | Archetipo | Righe | Priorità |
|---|---|---|---|---|
| `company` | **Soggetti** (+ viste Clienti/Fornitori/Gestori) | Lista + Scheda | 2 liv | **ALTA** |
| `engagement` + `phase` | **Commesse** (+ albero fasi/WBS, mock 24) | Lista + Scheda + Tree | 2 liv | **ALTA** |
| `asset` | **Asset / parco installato** | Lista + Scheda | 2 liv | MEDIA |
| `resource` | **Risorse** | Lista + Scheda | 2 liv | MEDIA |
| `activity` | **Attività / Agenda** | Lista + Scheda | — | MEDIA |
| `app_user` + `role` | **Utenti e ruoli** | Lista + Scheda | 2 liv | MEDIA |
| `capture` | **Catture (inbox)** | Lista | 2 liv | MEDIA |
| `lookup_value` | **Stati ed etichette** | Lista + CRUD | 1 liv | BASSA |
| `number_series` | **Numerazioni** | Lista + CRUD | 1 liv | BASSA |
| `plan`/`subscription` | **Piano e fatturazione** | Scheda/settings | — | BASSA |

> `material`/**Articoli** è coperto dal **Blocco C** (già allo standard v2). `work_order`/**Ordini di lavoro** è già v2 (Blocco B). Non rifarli qui.

**Ordine di esecuzione:** parti da **Soggetti** e **Commesse** (le più viste), **fermati e mostra a Ricardo** (checkpoint) per validare che il pattern v2 sia corretto, poi prosegui con le **MEDIA** in batch e infine le **BASSA**. Ogni batch chiude con `DONE_M_<entità>.md` + **screenshot prima/dopo** accostati al mock 41.

**DoD del blocco:** ogni entità dell'inventario rispetta i 10 punti della checklist v2; nessuna lista mostra più icone-azione sulle righe; nessun CRUD usa più il drawer master-detail; una **stessa** `EntityList` e una **stessa** `ObjectPage` sono riusate da tutte le entità (nessun copia-incolla divergente).
...
Apri `http://localhost:5173` (owner@fibra.demo) → **Lavoro → Ordini di lavoro** (oggi ancora etichettato "Ordinativi (FTTH)" finché non parte il Blocco B-ter), contro il mock 44:
