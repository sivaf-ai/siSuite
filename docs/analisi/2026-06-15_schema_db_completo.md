...
> Documento di **riferimento dello schema DB** al **15/06/2026**, generato dallo
> **schema reale** del database in esecuzione (`pg_dump --schema-only`), non dai file
> di migrazione. È lo **stato attuale** (migrazioni `001`…`023` applicate). PostgreSQL 16.
> Pensato per Claude AI e per i tecnici: **SQL standard**, autosufficiente.
>
> **Provenienza:** `pg_dump -U sisuite_admin -d sisuite --schema-only --no-owner --no-privileges -n public`,
> ripulito dal boilerplate di sessione. In testa sono stati aggiunti a mano i `CREATE EXTENSION`
> (pg_dump li omette quando si filtra per schema con `-n public`).
>
> **Esclusioni:** gli ENUM creati da **GoTrue** nello schema `public`
> (`aal_level`, `code_challenge_method`, `factor_status`, `factor_type`, `one_time_token_type`)
> sono stati rimossi: NON fanno parte di siSuite. Le **tabelle** di GoTrue vivono nello schema
> `auth` (non incluse). Resta `schema_migrations` (tabella di tracking di GoTrue) e
> `sisuite_migrations` (tracking delle nostre migrazioni).
>
> **Estensioni richieste:** `pgcrypto`, `btree_gist`, `vector` (pgvector, per l'embedding di `capture`).

## Migrazioni applicate (001→023)
- `001_schema_core.sql` — tabelle base + patch pre-sviluppo + seed (ruoli, stati canonici, etichette, piani).
- `002_rls_policies.sql` — Row-Level Security completa + funzioni helper di sessione.
- `003_app_functions.sql` — `app_resolve_context` (contesto utente per l'RLS).
- `004_field_definition.sql` — tabella `field_definition` (campi dinamici per entità/verticale) + seed.
- `005_field_definition_rls.sql` — RLS su `field_definition`.
- `006_fiber_fields.sql` — SOLO SEED: campi di sistema del verticale `fiber`.
- `007_term_override.sql` — tabella `term_override` (glossario di dominio per-tenant) + RLS.
- `008_cost_fields.sql` — SOLO SEED: campi `engagement.budget`, `resource.hourly_cost`, `material.unit_cost` (money).
- `009/010` — `engagement_template` creata e poi rimossa (duplicava `template`); 010 ripulisce i doppioni di 008.
- `011_time_typology.sql` — tipologia ore su lista.
- `012_time_entry_rates.sql` — tariffe fotografate + listino `rate_card`.
- `013_time_entry_approval.sql` — approvazione/blocco ore in blocco.
- `014_absences.sql` — assenze + saldi (`absence_entry`, `absence_balance`).
- `015_time_tracking.sql` — cronometro (`time_tracking_session`).
- `016_time_entry_context.sql` — vincolo di contesto sulle ore.
- `017_work_report.sql` — rapportino (`work_report`, `work_report_time_entry`).
- `018_agenda.sql` — `schedule_mode`/`pinned_day` su activity (integrazione motore gated).
- `019_engagement_budget.sql` — colonne `budget_amount`/`budget_minutes`/`budget_currency` su engagement.
- `020_stock_catalog_locations.sql` — magazzino: ubicazioni ad albero (`stock_location`).
- `021_stock_movements.sql` — movimenti immutabili + giacenze a media mobile (`stock_movement`, `stock_balance`).
- `022_stock_documents.sql` — documenti carico/trasferimento/rettifica (`stock_document`, `stock_document_line`).
- `023_lookup_override.sql` — `lookup_override` (personalizzazione per-tenant delle voci di sistema).

> **Inventario alla generazione (15/06/2026):** 42 tabelle · 7 enum siSuite · 12 funzioni · 74 indici · 34 trigger · 88 policy RLS.

---

```sql
-- =====================================================================
--  ESTENSIONI RICHIESTE (aggiunte a mano: pg_dump le omette con -n public)
-- =====================================================================
CREATE EXTENSION IF NOT EXISTS pgcrypto  WITH SCHEMA public;
CREATE EXTENSION IF NOT EXISTS btree_gist WITH SCHEMA public;
CREATE EXTENSION IF NOT EXISTS vector    WITH SCHEMA public;  -- pgvector 0.8.x
