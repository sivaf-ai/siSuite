...
| `field_definition.sql` | Catalogo dei campi dentro `attributes jsonb` (P.IVA, CF, dati tecnici per-verticale…): tabella + seed reali | Migrazione **003**. **Obbligatorio**: guida la validazione API e il rendering automatico dei form. |
| `FRONTEND_SPEC.md` | Spec frontend **vincolante**: componenti, CRUD completo, inventario campi, albero, icone, criteri di accettazione | **Ha precedenza** sulle parti UI di questo brief. Il frontend si fa seguendo quel documento. |
