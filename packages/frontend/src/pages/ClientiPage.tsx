...
export function ClientiPage() {
  ...
  const { data, loading, error } = useApi<ListResp>(`/companies?${params.toString()}`);
  ...
  const columns: ListColumn<CompanyDto>[] = [
    { key: 'name', header: 'Nome', sub: 'tipo', render: (r) => (
